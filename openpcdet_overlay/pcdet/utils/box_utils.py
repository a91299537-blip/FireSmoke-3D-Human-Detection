...
def get_rwiou(bboxes1, bboxes2, r_factor=1, voxel_size=(0.8, 0.8)):
    ...
    l1 = torch.exp(torch.clamp(bboxes1[..., 3], min=-5, max=5))
    w1 = torch.exp(torch.clamp(bboxes1[..., 4], min=-5, max=5))
    h1 = torch.exp(torch.clamp(bboxes1[..., 5], min=-5, max=5))
    l2 = torch.exp(torch.clamp(bboxes2[..., 3], min=-5, max=5))
    w2 = torch.exp(torch.clamp(bboxes2[..., 4], min=-5, max=5))
    h2 = torch.exp(torch.clamp(bboxes2[..., 5], min=-5, max=5))
    s1, c1 = bboxes1[..., 6], bboxes1[..., 7]
    s2, c2 = bboxes2[..., 6], bboxes2[..., 7]

    x1, y1 = x1u * voxel_size[0], y1u * voxel_size[1]
    x2, y2 = x2u * voxel_size[0], y2u * voxel_size[1]
    z1, z2 = z1u, z2u

    eps = 1e-4
    l1, w1, h1 = [torch.clamp(v, min=eps, max=30) for v in (l1, w1, h1)]
    l2, w2, h2 = [torch.clamp(v, min=eps, max=30) for v in (l2, w2, h2)]
    s1, c1 = torch.clamp(s1, -1, 1), torch.clamp(c1, -1, 1)
    s2, c2 = torch.clamp(s2, -1, 1), torch.clamp(c2, -1, 1)

    inter_l = torch.maximum(x1 - l1 / 2, x2 - l2 / 2)
    inter_r = torch.minimum(x1 + l1 / 2, x2 + l2 / 2)
    inter_t = torch.maximum(y1 - w1 / 2, y2 - w2 / 2)
    inter_b = torch.minimum(y1 + w1 / 2, y2 + w2 / 2)
    inter_u = torch.maximum(z1 - h1 / 2, z2 - h2 / 2)
    inter_d = torch.minimum(z1 + h1 / 2, z2 + h2 / 2)

    inter_volume = (
        torch.clamp(inter_r - inter_l, min=0)
        * torch.clamp(inter_b - inter_t, min=0)
        * torch.clamp(inter_d - inter_u, min=0)
    )
    inter_volume *= (
        (1 - r_factor * torch.abs(s1 - s2) / 2)
        * (1 - r_factor * torch.abs(c1 - c2) / 2)
    )

    c_l = torch.minimum(x1 - l1 / 2, x2 - l2 / 2)
    c_r = torch.maximum(x1 + l1 / 2, x2 + l2 / 2)
    c_t = torch.minimum(y1 - w1 / 2, y2 - w2 / 2)
    c_b = torch.maximum(y1 + w1 / 2, y2 + w2 / 2)
    c_u = torch.minimum(z1 - h1 / 2, z2 - h2 / 2)
    c_d = torch.maximum(z1 + h1 / 2, z2 + h2 / 2)
    ...
    union = l1 * w1 * h1 + l2 * w2 * h2 - inter_volume

    u = inter_diag / torch.clamp(c_diag, min=eps)
    rdiou = inter_volume / torch.clamp(union, min=eps)
    return u, rdiou
