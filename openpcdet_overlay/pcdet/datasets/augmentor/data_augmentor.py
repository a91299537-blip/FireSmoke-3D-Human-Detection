...
class DataAugmentor(object):
    ...
    def smoke_scene_degradation(self, data_dict=None, config=None):
        """Apply smoke-specific point degradation during training only."""
        if data_dict is None:
            return partial(self.smoke_scene_degradation, config=config)
        if np.random.random() > float(config.get('PROB', 1.0)):
            return data_dict

        data_dict['points'] = augmentor_utils.smoke_scene_degradation(
            data_dict['points'], data_dict['gt_boxes'], config
        )
        return data_dict
