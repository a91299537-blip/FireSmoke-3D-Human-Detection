...
def smoke_scene_degradation(points, gt_boxes, config, rng=None, return_stats=False):
    """Simulate smoke-degraded LiDAR returns without changing ground-truth boxes.

    The transform combines range/intensity-aware echo loss, target-centric BEV
    holes, and low-intensity backscatter points sampled along measured rays.
    It is intended for training-time use after geometric augmentation.
    """
    ...
    echo_cfg = config.get('ECHO_DROPOUT', {})
    if echo_cfg.get('ENABLED', True):
        rate_range = echo_cfg.get('DROP_RATE', [0.08, 0.22])
        base_rate = float(rng.uniform(rate_range[0], rate_range[1]))
        ranges = np.linalg.norm(points[:, :3], axis=1)
        range_scale = max(float(np.percentile(ranges, 95)), 1e-3)
        range_term = np.clip(ranges / range_scale, 0.0, 1.5)

        low_intensity_term = np.zeros(len(points), dtype=np.float32)
        if points.shape[1] > 3:
            intensity = points[:, 3].astype(np.float32)
            low, high = np.percentile(intensity, [5, 95])
            norm_intensity = np.clip((intensity - low) / max(float(high - low), 1e-6), 0.0, 1.0)
            low_intensity_term = 1.0 - norm_intensity

        drop_prob = base_rate * (
            1.0
            + float(echo_cfg.get('RANGE_WEIGHT', 0.35)) * range_term
            + float(echo_cfg.get('LOW_INTENSITY_WEIGHT', 0.20)) * low_intensity_term
        )
        drop_prob = np.clip(drop_prob, 0.0, float(echo_cfg.get('MAX_DROP_PROB', 0.65)))
        keep_mask = rng.random(len(points)) >= drop_prob

        min_points = min(int(echo_cfg.get('MIN_REMAIN_POINTS', 1024)), len(points))
        if int(keep_mask.sum()) < min_points:
            recover = rng.choice(np.flatnonzero(~keep_mask), min_points - int(keep_mask.sum()), replace=False)
            keep_mask[recover] = True

        stats['echo_dropped'] = int((~keep_mask).sum())
        points = points[keep_mask]

    center_cfg = config.get('CENTER_HOLE', {})
    if center_cfg.get('ENABLED', True) and len(points) > 0 and len(gt_boxes) > 0:
        remove_mask = np.zeros(len(points), dtype=bool)
        radius_range = center_cfg.get('RADIUS_RATIO', [0.18, 0.38])
        min_box_points = int(center_cfg.get('MIN_BOX_POINTS', 8))
        min_remain = int(center_cfg.get('MIN_REMAIN_POINTS', 4))
        box_prob = float(center_cfg.get('PROB', 0.65))

        for box in gt_boxes:
            if box.shape[0] < 7 or rng.random() > box_prob:
                continue
            dx, dy, dz = np.maximum(box[3:6], 1e-3)
            offset = points[:, :3] - box[:3]
            ...
            in_box = (
                ...
                & (np.abs(local_z) <= dz * 0.5)
                & (~remove_mask)
            )
            box_indices = np.flatnonzero(in_box)
            if len(box_indices) < min_box_points:
                continue

            radius_ratio = float(rng.uniform(radius_range[0], radius_range[1]))
            radius_x = max(dx * 0.5 * radius_ratio, 1e-3)
            radius_y = max(dy * 0.5 * radius_ratio, 1e-3)
            center_hole = in_box & ((local_x / radius_x) ** 2 + (local_y / radius_y) ** 2 <= 1.0)
            hole_indices = np.flatnonzero(center_hole)
            max_remove = max(0, len(box_indices) - min_remain)
            if len(hole_indices) > max_remove:
                hole_indices = rng.choice(hole_indices, max_remove, replace=False)
            remove_mask[hole_indices] = True

        stats['center_dropped'] = int(remove_mask.sum())
        points = points[~remove_mask]

    pseudo_cfg = config.get('SMOKE_PSEUDO_POINTS', {})
    if pseudo_cfg.get('ENABLED', True) and len(points) > 0:
        ratio_range = pseudo_cfg.get('RATIO', [0.04, 0.12])
        pseudo_ratio = float(rng.uniform(ratio_range[0], ratio_range[1]))
        num_pseudo = int(round(len(points) * pseudo_ratio))
        num_pseudo = min(num_pseudo, int(pseudo_cfg.get('MAX_POINTS', 12000)))
        if num_pseudo > 0:
            base_indices = rng.choice(len(points), num_pseudo, replace=num_pseudo > len(points))
            pseudo = points[base_indices].copy()
            radial_range = pseudo_cfg.get('RADIAL_SCALE', [0.35, 0.95])
            radial_scale = rng.uniform(radial_range[0], radial_range[1], size=(num_pseudo, 1))
            jitter_std = np.asarray(pseudo_cfg.get('XYZ_JITTER_STD', [0.03, 0.03, 0.02]), dtype=np.float32)
            pseudo[:, :3] = pseudo[:, :3] * radial_scale + rng.normal(
                0.0, jitter_std, size=(num_pseudo, 3)
            )
            if pseudo.shape[1] > 3:
                intensity_scale = pseudo_cfg.get('INTENSITY_SCALE', [0.05, 0.30])
                pseudo[:, 3] *= rng.uniform(
                    intensity_scale[0], intensity_scale[1], size=num_pseudo
                )
            points = np.concatenate([points, pseudo], axis=0)
            stats['pseudo_added'] = int(num_pseudo)
