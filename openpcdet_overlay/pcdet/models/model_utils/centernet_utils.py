...
def _topk_sparse(scores, batch_size, batch_idx, K=500):
    ...
    points_per_batch = [int((batch_idx == bidx).sum().item()) for bidx in range(batch_size)]
    topk_per_class = min(K, min(points_per_batch))

    for bidx in range(batch_size):
        score = scores[batch_idx == bidx].permute(1, 0)
        topk_scores, topk_inds = torch.topk(score, topk_per_class)
        flat_scores = topk_scores.reshape(-1)
        final_k = min(K, flat_scores.shape[0])
        topk_score, topk_ind = torch.topk(flat_scores, final_k)
        topk_classes = (topk_ind // topk_per_class).int()
        topk_inds = topk_inds.reshape(-1).gather(0, topk_ind)
    ...
    return (
        torch.stack(topk_score_list),
        torch.stack(topk_inds_list),
        torch.stack(topk_classes_list),
    )


def _gather_feat_sparse(feats, inds, batch_size, batch_idx):
    expanded_inds = inds.unsqueeze(-1).expand(inds.size(0), inds.size(1), feats.size(-1))
    feats_list = [
        feats[batch_idx == bidx].gather(0, expanded_inds[bidx])
        for bidx in range(int(batch_size))
    ]
...
def decode_bbox_from_sparse_heatmap(
    ...
    batch_size = int(sparse_indices[:, 0].max().item()) + 1
    ...
    angle = torch.atan2(rot_sin, rot_cos)
    xs = (
        (spatial_indices[:, :, -1:] + center[:, :, 0:1] + 0.5)
        * feature_map_stride * voxel_size[0] + point_cloud_range[0]
    )
    ys = (
        (spatial_indices[:, :, -2:-1] + center[:, :, 1:2] + 0.5)
        * feature_map_stride * voxel_size[1] + point_cloud_range[1]
    )
    ...
    if iou is not None:
        iou = _gather_feat_sparse(iou, inds, batch_size, batch_idx).squeeze(-1)
        iou = torch.clamp(iou, min=0, max=1)
    ...
    for bidx in range(batch_size):
        cur_mask = mask[bidx]
        pred_dict = {
            'pred_boxes': final_box_preds[bidx, cur_mask],
            'pred_scores': final_scores[bidx, cur_mask],
            'pred_labels': final_class_ids[bidx, cur_mask],
        }
        if iou is not None:
            pred_dict['pred_ious'] = iou[bidx, cur_mask]
        ret_pred_dicts.append(pred_dict)
