...
def _load_checkpoint_file(filename, map_location=None):
    try:
...
class Detector3DTemplate(nn.Module):
    ...
    def load_params_from_file(self, filename, logger, to_cpu=False, pre_trained_path=None):
        ...
        checkpoint = _load_checkpoint_file(filename, map_location=loc_type)
        model_state_disk = checkpoint['model_state']
        if not pre_trained_path is None:
            pretrain_checkpoint = _load_checkpoint_file(pre_trained_path, map_location=loc_type)
        ...
        if optimizer is not None:
            if 'optimizer_state' in checkpoint and checkpoint['optimizer_state'] is not None:
                ...
                if os.path.exists(optimizer_filename):
                    optimizer_ckpt = _load_checkpoint_file(optimizer_filename, map_location=loc_type)
